// JS/Ferramenta.js
import { API_BASE_URL } from './apiConfig.js'; // <--- 1. IMPORTAÇÃO ESSENCIAL
...
// --- CORREÇÃO: Função para exibir o nome do usuário ---
function displayUserName(lang) { 
    ...
    if (welcomeMessage && userNameElement && trans) { 
        ...
        // Verifica se userInfo e userInfo.nome existem
...
async function buscarUsuarioDaFerramenta(ferramentaId) {
    try {
        // CORRIGIDO: Usa API_BASE_URL em vez de localhost
        const res = await fetch(`${API_BASE_URL}/api/ferramentas/${ferramentaId}/usuario`);
        if (!res.ok) return null;
        return await res.json();
    } catch (err) {
        console.error(`Erro ao buscar usuário da ferramenta ${ferramentaId}:`, err);
        return null;
    }
}
...
async function carregarFerramentas() {
    ...
    try {
        ...
        if (grid) grid.innerHTML = `<p>${currentTrans?.errorLoadingTools || 'Erro ao carregar.'}</p>`;
    }
}
